...
namespace ChatManager.Models
{
    public class MessagesRepository : Repository<Message>
    {
        public Message Create(Message message)
        {
            try
            {
                message.WrittenDate = DateTime.Now;
                message.Id = base.Add(message);
                return message;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Add message failed : Message - {ex.Message}");
            }
            return null;
        }
        ...
        public override bool Update(Message message)
        {
            try {
                ...
                System.Diagnostics.Debug.WriteLine($"Update message failed : Message - {ex.Message}");
            }
            return false;
        }

        public override bool Delete(int messageId)
        {
            try
            {
                Message messageToDelete = DB.Messages.Get(messageId);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Remove message failed : Message - {ex.Message}");
                EndTransaction();
                return false;
            }
        }
        ...
        public List<Message> GetConversation(int userId, int otherId)
        {
            ...
            {
                if (
                    (message.FromUserId == userId && message.ToUserId == otherId) ||
                    (message.FromUserId == otherId && message.ToUserId == userId)
        ...
        public Message FindMessage(int id)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Find message failed : Message - {ex.Message}");
                return null;
            }
        }
